...
const userService = {
  getUserIdByNickname: async (nickname: string | undefined) => {
    const data = await fetcher<User>("get", `/kart/v1.0/users/nickname/${nickname}`);
    return data;
  },
  getSoloMatchListByUserId: async (accessId: string | undefined) => {
  ...
  getTeamMatchListByUserId: async (accessId: string | undefined) => {
    const data = await fetcher<MatchList>(
      "get",
